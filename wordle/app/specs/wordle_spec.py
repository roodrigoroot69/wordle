...
with describe(WordlePlayProcessor) as self:
    ...
    with context("when the user make their first attempt and guess a letter right"):
        with it(
            "should return the correct letter with 2 value and other with 3 value and add try"
        ):
    ...
    with context(
        "when the user make their first attempt and guess a letter right and the right position"
    ):
        with it(
            "should return the correct letter with 1 value and other with 3 value and add try"
        ):
            self.word_repository_mock.get_active_word.return_value = self.user_record
            self.cache_repository_mock.get_attempt.return_value = 1
    ...
    with context(
        "when the user make their first attempt and does not get any letter right"
    ):
    ...
    with context("when the user send the correct word"):
        with it("should return all letters with 1 and save winner"):
            ...
            expect(self.winner_repository_mock.save_winner.call_args).to(
                equal(call(user_id=1, word="verde"))
            )
